...
def get_assignment(
    ...
    response = {
        ...
        "reference_solution": assignment.reference_solution if current_user.role in ["professor", "admin"] else None
    }
...
async def update_assignment(
    ...
    if reference_solution_file:
        ...
        # Generate unique file name
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        file_name = f"{assignment.course_id}_{timestamp}_{reference_solution_file.filename}"
        reference_solution_file_path = f"uploads/reference_solutions/{file_name}"
